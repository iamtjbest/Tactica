...
def get_primary_pos(pos):
    return (pos or "MF").split(",")[0].strip()
...
def select_starting_xi(team_name, formation):
    ...
    roster    = sorted(roster, key=lambda x: (x["Min"], x["G_A"]), reverse=True)
    xi, named = [], set()

    def draft_primary(pos, n):
        c = 0
        for p in roster:
            if c >= n: break
            if get_primary_pos(p["Pos"]) == pos and p["Name"] not in named:
                xi.append(p); named.add(p["Name"]); c += 1
        return c
    ...
    def draft_any(pos, n):
        ...
        for p in roster:
            if c >= n: break
            if pos in p["Pos"] and p["Name"] not in named:
                xi.append(p); named.add(p["Name"]); c += 1
        return c

    if draft_primary("GK",1) < 1: draft_any("GK",1)
    n = draft_primary("DF", def_count); draft_any("DF", def_count - n) if n < def_count else None
    n = draft_primary("MF", mid_count); draft_any("MF", mid_count - n) if n < mid_count else None
    n = draft_primary("FW", att_count); draft_any("FW", att_count - n) if n < att_count else None

    for p in roster:
        if len(xi) >= 11: break
        if p["Name"] not in named:
            p = dict(p); p["fallback"] = True
            xi.append(p); named.add(p["Name"])
    return xi
            ...
            if not my5 and not opp5:
                ...
                if xi:
                    for p in xi:
                        ...
                        pos  = get_primary_pos(p['Pos'])
                        st.markdown(
                            f"<div class='player-card'>"
                            f"<span><span class='pos-badge'>{pos}</span>{p['Name']}{warn}</span>"
                            f"<span class='stat-text'>⏱ {p['Min']} mins &nbsp;⚽ {ga} G+A</span>"
